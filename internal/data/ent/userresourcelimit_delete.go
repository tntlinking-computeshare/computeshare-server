// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/tntlinking-computeshare/computeshare-server/internal/data/ent/predicate"
	"github.com/tntlinking-computeshare/computeshare-server/internal/data/ent/userresourcelimit"
)

// UserResourceLimitDelete is the builder for deleting a UserResourceLimit entity.
type UserResourceLimitDelete struct {
	config
	hooks    []Hook
	mutation *UserResourceLimitMutation
}

// Where appends a list predicates to the UserResourceLimitDelete builder.
func (_d *UserResourceLimitDelete) Where(ps ...predicate.UserResourceLimit) *UserResourceLimitDelete {
	_d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (_d *UserResourceLimitDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, _d.sqlExec, _d.mutation, _d.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *UserResourceLimitDelete) ExecX(ctx context.Context) int {
	n, err := _d.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (_d *UserResourceLimitDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(userresourcelimit.Table, sqlgraph.NewFieldSpec(userresourcelimit.FieldID, field.TypeUUID))
	if ps := _d.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, _d.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	_d.mutation.done = true
	return affected, err
}

// UserResourceLimitDeleteOne is the builder for deleting a single UserResourceLimit entity.
type UserResourceLimitDeleteOne struct {
	_d *UserResourceLimitDelete
}

// Where appends a list predicates to the UserResourceLimitDelete builder.
func (_d *UserResourceLimitDeleteOne) Where(ps ...predicate.UserResourceLimit) *UserResourceLimitDeleteOne {
	_d._d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query.
func (_d *UserResourceLimitDeleteOne) Exec(ctx context.Context) error {
	n, err := _d._d.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{userresourcelimit.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *UserResourceLimitDeleteOne) ExecX(ctx context.Context) {
	if err := _d.Exec(ctx); err != nil {
		panic(err)
	}
}
