// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/tntlinking-computeshare/computeshare-server/internal/data/ent/cyclerecharge"
	"github.com/tntlinking-computeshare/computeshare-server/internal/data/ent/predicate"
)

// CycleRechargeDelete is the builder for deleting a CycleRecharge entity.
type CycleRechargeDelete struct {
	config
	hooks    []Hook
	mutation *CycleRechargeMutation
}

// Where appends a list predicates to the CycleRechargeDelete builder.
func (_d *CycleRechargeDelete) Where(ps ...predicate.CycleRecharge) *CycleRechargeDelete {
	_d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (_d *CycleRechargeDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, _d.sqlExec, _d.mutation, _d.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *CycleRechargeDelete) ExecX(ctx context.Context) int {
	n, err := _d.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (_d *CycleRechargeDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(cyclerecharge.Table, sqlgraph.NewFieldSpec(cyclerecharge.FieldID, field.TypeUUID))
	if ps := _d.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, _d.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	_d.mutation.done = true
	return affected, err
}

// CycleRechargeDeleteOne is the builder for deleting a single CycleRecharge entity.
type CycleRechargeDeleteOne struct {
	_d *CycleRechargeDelete
}

// Where appends a list predicates to the CycleRechargeDelete builder.
func (_d *CycleRechargeDeleteOne) Where(ps ...predicate.CycleRecharge) *CycleRechargeDeleteOne {
	_d._d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query.
func (_d *CycleRechargeDeleteOne) Exec(ctx context.Context) error {
	n, err := _d._d.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{cyclerecharge.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *CycleRechargeDeleteOne) ExecX(ctx context.Context) {
	if err := _d.Exec(ctx); err != nil {
		panic(err)
	}
}
